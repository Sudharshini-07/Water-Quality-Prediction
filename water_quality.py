...
def fetch_realtime_data():
    try:
        # API for real-time water quality (multiple parameters)
        url = "https://www.waterqualitydata.us/data/Result/search?" + \
              "countrycode=US&statecode=US:CA&" + \
              "characteristicName=pH&characteristicName=Hardness&" + \
              "characteristicName=Conductivity&characteristicName=Turbidity&" + \
              "startDateLo=2024-01-01&mimeType=csv"
        ...
        st.warning(f"Error fetching real-time data: {str(e)}")
        return None
...
with tab2:
    ...
    st.markdown("""
    This feature retrieves actual water quality measurements from USGS monitoring stations.
    Note that not all parameters may be available in real-time data.
    """)
    
    if st.button("🌍 Get Latest Water Quality Data"):
        ...
        if realtime_data is not None and not realtime_data.empty:
            ...
            st.warning("Note: Some parameters were estimated using median values where real-time data was unavailable.")
        else:
            st.error("Could not fetch real-time data. Please try again later.")
...
*Using Random Forest Classifier and real-time USGS water quality data*  
